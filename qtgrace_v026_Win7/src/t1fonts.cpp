...
/// qDebug() << "Font=" << FontID << "String=" << cs->s << "Len=" << len << "Space=" << Space << "mod=" << modflag << "size=" << Size;
...
//qDebug() << "WriteString in t1fonts: cs=#" << cs->s << "# font=" << cs->font;
        /* Post-process the CS */
...
//qDebug() << "Write String alpha=" << def_alpha << " String=" << theString;
        ...
        {
            /// qDebug() << "cs.s=" << QString::fromUtf8(cs->s) << "cs=" << cs << " cs.len=" << cs->len << "Page_dpv=" << page_dpv;
...
/// if (glyph==NULL)  qDebug() << "WriteString: GLYPH==NULL";
        if (glyph != NULL) {
