...
static view v;
static world wtmp,wtmp2,*wtmp3=NULL;
...
#endif
//qDebug() << "Is Double Click=" << dbl_click << " timer_running=" << mainWin->mainArea->scroll->event_timer.isActive();
            switch (action_flag)
            {
            case DO_NOTHING:
                ...
                mainWin->mainArea->scroll->haltEvent();
                    ...
                    {
                        get_page_viewport(&bb.xv2,&bb.yv1);
                        anchor_vp.x=bb.xv2;
                        anchor_vp.y=bb.yv1;
            ...
            case ZOOM_2ND:
                ...
                SaveTickmarksStatesPrevious(X_AXIS,Y_AXIS,nr_of_sel_graphs,sel_graphs);
                    ...
                    get_graph_world(get_cg(), &wtmp);//get unzoomed axes scale
                        ...
                        {
                        wtmp.yg1=zoom_donut1;
                        wtmp.yg2=zoom_donut2;
                        }
                        else//pie-slice --> Phi-scaling
                        {
                        wtmp.xg1=zoom_donut1;
                        wtmp.xg2=zoom_donut2;
                        }
//qDebug() << "x1=" << wtmp.xg1/M_PI*180.0 << " x2=" << wtmp.xg2/M_PI*180.0 << " y1=" << wtmp.yg1 << " y2=" << wtmp.yg2;
                    set_graph_world(get_cg(),wtmp);
                    }
                    else
                    {
 ...
 */
                if (context_menu_in_main)
                {
                    if (find_point(cg, vp, &track_setno, &loc) == RETURN_SUCCESS)
                    {
                        ...
                        prepare_highlight_segments_with_sets(1,sel_graphs,sel_graphs+1);
